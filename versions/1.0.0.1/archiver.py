...
def ocr_video_frames(media: str, duration: float, extra_times: list,
                     log, max_frames: int = 10) -> list:
    """Текст из видеоряда (надписи, названия, ссылки на экране).
    Возвращает [(секунда, строка)]. Нужна vision-модель Ollama."""
    try:
        import analyzer
        if not analyzer.vision_model():
            log("Vision-модель не найдена (ollama pull llava:7b) — "
                "текст из видеоряда пропускаю.")
            return []
    except Exception:  # noqa: BLE001
        return []
    times = []
    if duration:
        step = max(15, int(duration) // max_frames or 15)
    ...
    if not times:
        return []
    log(f"Читаю текст с экрана ({len(times)} кадров, vision-модель)...")
    seen, out = set(), []
    ...
    try:
        ...
        for t in times:
            ...
            if not grab_frame(media, t, fp):
                continue
            txt = analyzer.ocr_image(fp)
            for line in (txt or "").splitlines():
                line = line.strip(" \t-•*—")
                low = line.lower()
                if (len(line) >= 3 and low not in seen
                        and not low.startswith(("текста нет", "нет текста",
                                                "на изображении"))):
                    seen.add(low)
                    out.append((t, line))
    finally:
...
def process_video(url: str, out_root: str, log, model_size: str,
    ...
    try:
        ...
        shot_times = [t for t, _, _ in extra_shots] + \
            [t for t, _, _ in local_shots] if analysis else \
