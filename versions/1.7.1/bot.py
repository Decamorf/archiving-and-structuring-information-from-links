...
class TelegramBot:
    ...
    def _send_document(self, path, caption=""):
        ...
        # Telegram Bot API: обычные боты шлют файлы до 50 МБ
        if size_mb > 50:
            self._send(f"Архив {size_mb:.0f} МБ — больше лимита Telegram "
                       f"(50 МБ). Файл сохранён на компьютере, но не "
                       f"отправлен. Попробуйте без копии видео.")
            return
        last = None
        for attempt in range(3):
            try:
                ...
                last = e
                self.applog(f"отправка не удалась (попытка {attempt+1}/3), "
                            f"повтор...")
    ...
    def _handle_url(self, url):
        ...
        try:
            ...
            def log(msg):
                self.applog(msg)
                # в чат уходят все те же строки, что и в журнал на ПК
                self._send(str(msg))
