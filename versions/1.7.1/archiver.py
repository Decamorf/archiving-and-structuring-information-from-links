...
def sanitize(name: str, maxlen: int = 100) -> str:
    ...
    if not name:
        return "без_названия"
    name = strip_cjk(name)  # убираем китайские/японские иероглифы из имён
    name = re.sub(r'[\\/:*?"<>|\r\n\t#]', ' ', name)
...
def strip_cjk(s: str) -> str:
    """Удаляет китайские/японские/корейские иероглифы из текста-страховки
    (модель иногда сваливается в них). Оставляет остальной текст как есть.
    Пустые скобки и лишние пробелы после удаления подчищаются."""
    if not s:
        return s
    out = []
    for ch in s:
        o = ord(ch)
        cjk = (0x3000 <= o <= 0x9FFF or 0xAC00 <= o <= 0xD7AF
               or 0xF900 <= o <= 0xFAFF or 0xFF00 <= o <= 0xFFEF
               or 0x3040 <= o <= 0x30FF)
        if not cjk:
            out.append(ch)
    t = "".join(out)
    t = re.sub(r"\(\s*\)", "", t)          # пустые скобки
    t = re.sub(r"[ \t]{2,}", " ", t)
    t = re.sub(r" *([,.;:]) *\1+", r"\1", t)
    return t.strip()
