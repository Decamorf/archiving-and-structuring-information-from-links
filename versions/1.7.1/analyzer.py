...
def _strip_cjk(s):
    """Удаляет CJK-иероглифы (страховка от сваливания модели в китайский)."""
    if not s:
        return s
    out = [ch for ch in str(s) if not (
        0x3000 <= ord(ch) <= 0x9FFF or 0xAC00 <= ord(ch) <= 0xD7AF
        or 0xF900 <= ord(ch) <= 0xFAFF or 0xFF00 <= ord(ch) <= 0xFFEF
...
"tags": ["3-6 коротких тегов-ключевых слов ТОЛЬКО на русском или английском (латиницей): конкретные технологии, устройства, программы, темы; строчными, без решётки"]}}

ВАЖНО: title и tags писать ИСКЛЮЧИТЕЛЬНО русскими или латинскими буквами. НЕ используй китайские, японские или другие иероглифы ни при каких условиях, даже если в исходнике они есть."""
...
Правила: строго по делу, без воды и слов-паразитов, ничего не выдумывай — только то, что есть в транскрипте. Иностранные слова, названия программ, брендов, сервисов, устройств и команды пиши в оригинале ЛАТИНИЦЕЙ (Steam Deck, Docker, Windows), не транслитерируй кириллицей.

КРИТИЧЕСКИ ВАЖНО ПРО ЯЗЫК: весь конспект пиши ТОЛЬКО на русском языке (термины и названия — латиницей). Категорически НЕ используй китайские, японские, корейские или любые другие иероглифы — ни в одном слове, даже если исходное видео на другом языке. Если материал не на русском — переведи смысл на русский. Ответ — только Markdown конспекта, без вступлений."""
    if kind == "статья":
        return f"""Сделай конспект статьи «{title}» ({author}) для базы знаний.
Текст статьи:
{body}

Markdown-структура: ## Суть (2-3 предложения), ## Главное (тезисы и цифры списком), ## Команды и настройки (блок ``` если есть код/команды, иначе пропусти), ## Инструменты и ссылки. Подробно, но строго по делу, ничего не выдумывай. Весь текст — ТОЛЬКО на русском языке (термины латиницей), без китайских/японских иероглифов. Ответ — только Markdown."""
...
def analyze(kind: str, title: str, author: str, description: str,
    ...
    try:
        ...
        for k in ("topic", "subtopic", "title"):
            if str(meta.get(k) or "").strip():
                data[k] = _strip_cjk(str(meta[k]).strip())
    ...
    try:
        ...
        data["summary_md"] = _strip_cjk(summary.strip())
    except Exception as e:  # noqa: BLE001
...
Требования: МАКСИМАЛЬНО точный перевод; ничего не добавлять, не сокращать и не пропускать; сохранить разметку Markdown, заголовки, списки, таймкоды [мм:сс] и структуру абзацев; код, команды и содержимое `бэктиков` и блоков ``` НЕ переводить; имена собственные, названия программ, брендов и сервисов оставить в оригинале латиницей. Весь результат — ТОЛЬКО на языке «{target_lang}», без китайских/японских/корейских иероглифов.
Текст:
{ch}

Ответ — только перевод, без пояснений."""
        out.append(_strip_cjk(_ask(model, prompt, temperature=0.1)))
