...
"title": "чёткое информативное название заметки на русском"}}"""
...
Правила: строго по делу, без воды и слов-паразитов, ничего не выдумывай — только то, что есть в транскрипте. Ответ — только Markdown конспекта, без вступлений."""
    if kind == "статья":
...
{{"screenshots": [{{"n": 1, "time": секунды_от_начала_видео, "caption": "что видно на кадре и зачем"}}]}}
Время бери из таймкодов транскрипта и прибавь 2-3 секунды. Максимум 8 моментов, только реально полезные (интерфейс, настройки, схемы). Если таких нет — {{"screenshots": []}}"""
